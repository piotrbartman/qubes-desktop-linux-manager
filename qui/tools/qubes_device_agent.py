...
class VMListModeler:
    ...
    def _create_entries(self, options):
        for name, vm in self._domains_info.items():
            if name.startswith("@dispvm:"):
                ...
                vm_name = name
                prefix = ""
            # sanitize_domain_name(vm_name, assert_sanitized=True) TODO
...
class RPCConfirmationWindow:
    ...
    def __init__(
        ...
        # sanitize_domain_name(source, assert_sanitized=True)  TODO
        # sanitize_service_name(source, assert_sanitized=True)  TODO
    ...
    def _new_vm_list_modeler(self, options):
        return VMListModeler(options, self._entries_info)
